...
class MBDynAdapter:
    def __init__(self, mbdHelper, configFileName="../precice-config.xml"):
        ...
        self.interface = precice.Interface(
            "Structure_Solver", 0, 1)  # proc no, nprocs
        ...
        self.interface.set_mesh_vertices(nmeshID, self.nnodes, np.ravel(
            nodes[:, :self.dim]), self.nodeVertexIDs)
        ...
        if (self.interface.is_action_required(precice.action_write_initial_data())):
            self.interface.write_block_vector_data(
                self.displacementsID, self.nnodes, self.nodeVertexIDs, self.displacements)
        ...
        if (self.interface.is_read_data_available()):
            self.interface.read_block_vector_data(
                self.forceID, self.nnodes, self.nodeVertexIDs, self.force)
    ...
    def runPreCICE(self):
        ...
        while (self.interface.is_coupling_ongoing()):
            ...
            self.interface.write_block_vector_data(
                self.displacementsID, self.nnodes, self.nodeVertexIDs, np.ravel(relDisplacements))
            self.interface.advance(self.dt)
            self.interface.read_block_vector_data(
                self.forceID, self.nnodes, self.nodeVertexIDs, self.force)

            # i.e. not yet converged
            if (self.interface.is_action_required(precice.action_read_iteration_checkpoint())):
                self.interface.fulfilled_action(
