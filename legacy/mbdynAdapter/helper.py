...
class MBDynHelper:
    ...
    def initializeMBDyn(self):
        ...
        self.process = Popen(['mbdyn', '-f', self.mbdscript],
                             stdout=self.structurelogfile, stderr=self.structurelogfile)
        self.process.stdin = ''
        # self.process = Popen(['mbdyn','-f', self.mbdscript, '-o', self.caseName])
        self.nodal = mbcNodal(path, host, port, timeout, verbose, data_and_next, refnode, len(
            self.mesh.nodes), labels, rot, accels)
        self.nodal.negotiate()
        self.nodal.recv()
        self.initialized = True
        print('MBDyn initialized\ncontrolDict {} \nmaterialDict {}'.format(
            self.controlDict, self.materialDict))
    ...
    def readMsh(self, fileName):
            ...
            for line in fin:
                if '$PhysicalNames' in line:
                    ...
                    for l in lines_gen:
                        cells = l.split()
                        names.append(
                            (int(cells[0]), int(cells[1]), cells[2].replace('"', '')))
                    readNames = False
                elif readNodes:
                    nlines = int(line)
                    nodes = np.genfromtxt(itertools.islice(
                        fin, 0, nlines), dtype=float)
                    readNodes = False
                elif readElements:
                    nlines = int(line)
                    lines_gen = itertools.islice(fin, 0, nlines)
        ...
        for n in self.mesh.names:
            nedges = len(self.mesh.edges[np.where(
                self.mesh.edgeNames == n[1])])
            nmembranes = len(self.mesh.membranes[np.where(
                self.mesh.membraneNames == n[1])])
            print('{:<15} {:<10} {:<10}'.format(n[2], nedges, nmembranes))
        print('{:-^36}'.format(''))
        print('{:<15} {:<10} {:<10}'.format('Sum', len(
            self.mesh.edges), len(self.mesh.membranes)))
    ...
    def getCellCenters(self):
        if self.initialized:
            ...
            n = self.mesh.nodes
        tris = np.append(
            self.mesh.membranes[:, :3], self.mesh.membranes[:, [0, 2, 3]], axis=0)
    ...
    def calcLoads(self):
        ...
        memforces = triforces[:len(self.mesh.membranes)] + \
            triforces[len(self.mesh.membranes):]
    ...
    def solveStatic(self, tolerance=1e-6, maxIterations=10000, write=True):
        ...
        for i in range(maxIterations):
            ...
            previousX = currentX
            print('Finished iteration: {}/{}, displacement two-norm diff: {}/{}'.format(i,
                                                                                        maxIterations, twoNormX, tolerance))
    ...
    def writeVTK(self, extension=False):
        ...
        membranesstr = '\n'.join(['4 {} {} {} {}'.format(*m)
                                  for m in self.mesh.membranes])
        typestr = '\n'.join(
            np.array(9*np.ones(len(self.mesh.membranes), dtype=int), dtype=str))
        displacements = self.getDisplacements()
        displacementsstr = '\n'.join(
            ['{} {} {}'.format(*d) for d in (displacements)])
        stressesstr = ''
        if os.path.isfile('{}.pla'.format(self.caseName)):
            ...
            with open('{}.pla'.format(self.caseName)) as fin:
                ...
                memStresses = np.reshape(
                    np.array(memStresses[::-1], dtype=float), (-1, 4, 3))
        ...
        if len(self.nodalForces):
            nodalForcesstr = '\n'.join(
                ['{} {} {}'.format(*f) for f in (self.nodalForces)])
            forcesstr = '\n'.join(['{} {} {}'.format(*f)
                                   for f in (self.membraneForces)])
        if extension:
...
'''.format(self.MBNodes(), self.MBJoints(), damping, mass, self.MBMembranes(), self.MBForces()))
    ...
    def MBNodes(self):
        ...
        string = '\n'.join([nodesstr.format(i, n[0], n[1], n[2])
                            for i, n in enumerate(self.mesh.nodes)])
        if 'C' in self.materialDict and self.materialDict['C']:
            string += '\nstructural: {}, static, 0.0, 0.0, -1.0, eye, null, null, output, no;'.format(
                len(self.mesh.nodes))
    ...
    def MBDamping(self):
        ...
        string += '\n'.join(['joint: {}, deformable displacement joint, {}, null, {}, null, linear viscous, C;'.format(
            index+i, clampnode, i) for i in range(len(self.mesh.nodes))])
        string += '\n\n'
        return string
    ...
    def MBJoints(self):
        ...
        for name in self.mesh.names:
            if 'fix' in name[2]:
                iedges = self.mesh.edges[np.where(
                    self.mesh.edgeNames == name[1])]
                imembranes = self.mesh.membranes[np.where(
                    self.mesh.membraneNames == name[1])]
                idofs = np.unique(
                    np.append(np.ravel(iedges), np.ravel(imembranes)))
                if 'All' in name[2]:
                    dofs[idofs] = True
                if 'X' in name[2]:
                    dofs[idofs, 0] = True
        ...
        for i, d in enumerate(dofs):
            ...
            if d[2]:
                dof[2] = 'active'
            mbjoints.append(jointsstr.format(
                index + i, i, n[0], n[1], n[2], *dof))
        string = '\n'.join(mbjoints)
        return string
    ...
    def MBMass(self):
        ...
        areas = areas[:len(self.mesh.membranes)] + \
            areas[len(self.mesh.membranes):]

        # Preferrably no looping
        for i, m in enumerate(self.mesh.membranes):
            masses[m] += areas[i] / 4.0

        string = '\n'.join([massstr.format(index + i, i, m)
    ...
    def MBMembranes(self):
        ...
        if len(self.preStress):
            ...
            string = '\n'.join([membranesstr.format(index + i, m[0], m[1], m[2], m[3], *ps)
                                for i, (m, ps) in enumerate(zip(self.mesh.membranes, self.preStress))])
        else:
            membranesstr = "membrane4eas: {}, {}, {}, {}, {}, isotropic, E, Et, nu, nut, thickness, t;"
            string = '\n'.join([membranesstr.format(
                index + i, m[0], m[1], m[2], m[3]) for i, m in enumerate(self.mesh.membranes)])
        return string
